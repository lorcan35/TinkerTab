...
/* turn_id width is owned by the dictation FSM (DICT_TURN_ID_LEN); keep the
 * voice.c alias in lockstep so the wire frames and the FSM-minted id agree
 * byte-for-byte (W1 — one generator, no split-brain). */
#define TURN_ID_LEN DICT_TURN_ID_LEN
_Static_assert(TURN_ID_LEN == 13, "turn_id width changed — update both sides");
static char s_current_turn_id[TURN_ID_LEN] = {0};

static void gen_turn_id(void) {
   /* Delegate to the single generator in voice_dictation.c so a non-dictation
    * turn (ask/call/solo) mints ids identically to a dictation turn.  Preserve
    * the obs side-effect. */
   voice_turn_id_gen(s_current_turn_id);
...
esp_err_t voice_start_dictation(void)
{
   ...
   if (s_mic_running) {
    ...
    if (offline) {
        ...
        * refuses (returns NULL) if a turn of another origin (e.g. an offline
        * REST upload) is in flight, closing the WS↔offline collision (S1-3). */
       const char *tid = voice_dictation_begin(DICT_ORIGIN_WS, NULL, voice_dictation_now_ms());
       if (!tid) {
          ESP_LOGW(TAG, "voice_start_dictation refused — dictation pipeline busy");
          ui_home_show_toast("Busy transcribing — try again in a moment");
          voice_modes_set_internal(VOICE_MODE_ASK);
          return ESP_ERR_INVALID_STATE;
       }
       /* Keep voice.c's wire id in lockstep with the FSM-minted id. */
       strncpy(s_current_turn_id, tid, sizeof s_current_turn_id - 1);
       s_current_turn_id[sizeof s_current_turn_id - 1] = '\0';
       tab5_debug_obs_event("turn.start", s_current_turn_id);
       char frame[128];
...
esp_err_t voice_stop_listening(void)
{
   ...
   if (offline_dictate) {
      ...
      /* W1: the live recording is now saved to SD as a RECORDED note; return
       * the dictation FSM to IDLE.  The transcription queue OWNS the later
       * upload — it claims the FSM via voice_dictation_try_begin_offline
       * (IDLE→UPLOADING, origin=OFFLINE) when it actually POSTs.  (Pre-W1 this
       * pre-set UPLOADING, which would now leave the FSM unclaimable.) */
