...
### K144 cold-start model load can hang for 5+ min (or indefinitely)
- **Date:** 2026-04-29
- **Symptom:** Phase 3 of the K144 LLM Module integration verified setup-ack + error-path but couldn't complete a clean inference round-trip.  After Tab5 sends `llm.setup`, the K144 returns the early ack with `work_id=llm.NNNN` instantly (Tab5 sees `error.code=0`, work_id assigned), but the ACTUAL model load (mapping the qwen2.5-0.5B-prefill-20e checkpoint into NPU memory) runs asynchronously after the ack and takes **4-5 minutes minimum** on a cold boot.  Tab5's inference request sent during this window is silently dropped on the K144 side — no error response, no streaming chunks, just a 30-sec UART timeout on the Tab5 side.  In one observed case the model load hung for 18+ minutes without completing despite the K144 process being alive (`State: S sleeping`, `load avg: 3.5+`, no journal log progress past `Init: LLM init start`).
...
- **Operational workarounds:**
  - **Hard-reboot the K144** via `sudo adb shell reboot` between debugging sessions to get a clean state.  Restart of just the `llm-llm` service is not always sufficient — the NPU may retain stale mappings that block the next load.
  - **First inference after K144 boot is the slow one** (~5 min cold-start); plan to issue setup at K144 power-up and warm-run a throwaway prompt before the user notices.  Phase 4/5 of `docs/PLAN-m5-llm-module.md` should consider doing a background warm-up at Tab5 boot once it knows the K144 is present.
  - **Watch `/proc/loadavg` on the K144 via ADB** during debugging — load > 1.0 sustained means the LLM is still loading; load < 0.5 means it's idle (model loaded OR no work in progress).
