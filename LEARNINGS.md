...
### K144 chain hardening (audit 2026-04-29) — 7-wave program closes 14/18 audit findings
- **Date:** 2026-04-29
- **Context:** After Phase 6b shipped the autonomous K144 voice-assistant chain (commits `4ec33dc` + `3a6f67a`), three parallel audits (UI/UX, piping/data-flow, architecture) surfaced 5 P0s + ~12 P1s + multiple P2 polish items.  See `docs/AUDIT-k144-chain-2026-04-29.md` and `docs/PLAN-k144-chain-hardening.md`.  Tracked in TT #327, all landed on PR #326 across waves 1-7:
  - **Wave 1** — Port C UART recursive mutex (`bsp/tab5/uart_port_c`); every public `voice_m5_llm_*` entry serialises; chain_run takes/releases per outer loop iteration so a concurrent text turn lands between frames.  `s_rx_len` reset + UART flush on chain entry/teardown to prevent stale frames carrying over.  `voice_send_text` refuses with toast when chain active to close the silent double-fire path.
  - **Wave 2** — `VOICE_MODE_COUNT 4 → 5` plus 5 hardcoded `[4]` arrays grew to `[VOICE_MODE_COUNT]` with new "Onboard" cells.  `TH_MODE_ONBOARD = 0x8E5BFF` violet tint distinguishes from the four prior modes.  Chat header chip + mode-cycle long-press now reach vmode=4.  Mic-mute respected as a privacy switch in chain branch.  Home pill suppresses "NO DRAGON" / "Reconnecting…" when in vmode=4 + K144 ready (shows "ONBOARD" instead).
  - **Wave 3** — Stripped `tts.setup` from chain (K144's `single_speaker_english_fast` SummerTTS crashed mid-utterance with an Eigen reshape assertion).  Per-utterance synth via `voice_m5_llm_tts` posted to `tab5_worker` on every LLM `finish=true`; both serialise on Wave 1's UART mutex.  `ui_chat_hide` calls `voice_stop_listening` if chain active to close the chain-leak-on-nav path.
  - **Wave 4a** — `stop_flag` plumbed through `chain_setup_unit`'s outer + inner loops so user-stop during the K144's 5-15 sec NPU cold-start bails fast.  `on_ball_tap` now checks `voice_onboard_chain_active()` first so PROCESSING-state taps still route to chain-stop (was previously dropped with "ball tap ignored").
  - **Wave 4b (architectural)** — Extracted `main/voice_onboard.{c,h}`: ~700 lines moved out of voice.c.  Owns the entire vmode=4 surface (boot warmup + per-text-turn failover + autonomous chain).  `voice_set_state` promoted from file-static to public (`voice.h`) so the new module can drive state transitions.  voice.c shrunk 4316 → 3712 LOC.  No behavior change; verified with `story_smoke 14/14 + story_onboard 14/14`.
  - **Wave 5** — `GET /m5` debug endpoint (chain_active, failover_state, uart_baud) closes the diagnostic gap where "chain didn't start" required serial+adb.  Obs events at lifecycle transitions (`m5.warmup` start/ready/unavailable, `m5.chain` start/stop) feed the existing `/events` ring + e2e harness.  New `story_onboard.py` e2e scenario.
  - **Wave 6 (UX parity)** — Chain LLM tokens now stream into a TINKER bubble character-by-character via `voice_set_llm_text` setter.  poll_voice in ui_chat polls `voice_get_llm_text()` during PROCESSING/SPEAKING and runs `chat_msg_view_begin/append/end_streaming` lifecycle — same code path Dragon uses, so chain replies build the bubble live instead of commit-on-finish.  Chat input pill smooth-slides on keyboard open/close (220 ms ease-out anim) instead of teleporting.
  - **Wave 7 (polish)** — Setup-failure toast maps `ESP_ERR_TIMEOUT` / `ESP_ERR_INVALID_RESPONSE` / `ESP_ERR_INVALID_STATE` / `ESP_ERR_NO_MEM` to specific actionable messages ("K144 not responding — power-cycle?", "K144 busy — restart services?", silent on user-stop, "Out of memory…").  `chain_uptime_ms` exposed via `/m5` so a stuck chain is observable remotely.  Removed dead `onboard_audio_callback` (post-Wave-3a chain has no `tts.setup`; chain_run accepts NULL audio_cb).
- **Findings closed:** 14 of 18 audit P0/P1 items.  Remaining P2 polish (LLM streaming was actually closed in Wave 6, OCP gap on chain dispatch hardcoded to 4 units, audio_cb dispatch refactor — already mitigated by Wave 3a moving TTS off the drain path) tracked in the plan doc as future-PR backlog.
- **Methodology takeaway:** the wave-by-wave shape worked.  Wave 1 unblocked Wave 3's TTS workaround; Wave 4b's extract was decoupled and parallelisable; Wave 5's `/m5` made later debugging trivial.  Each wave shipped with `story_smoke + story_onboard` regression checks on hardware before the next started.  The deferral pattern (Wave 4b → focused follow-up; Wave 6 LLM streaming was originally Wave 3b, deferred when Wave 3 grew too big) kept individual PRs reviewable.  When a wave turned out to be already mitigated by an earlier wave (audit #4 audio_cb), we documented the mitigation rather than building it twice.
- **Prevention:**
...
## TT #247 Wave 17 — `lv_obj_clean` is the third option between destroy/recreate and naive hide/show

- **Date:** 2026-05-01
- **Symptom:** Camera screen has ~30 LVGL widgets backed by a 1.8 MB PSRAM canvas buffer.  Every navigate-into-camera + navigate-out cycle did `lv_obj_delete(scr_camera)` + `free_canvas_buffer()`, churning 30 widget allocs + 30 widget frees + 1 PSRAM alloc + 1 PSRAM memset through the TLSF allocator.  Under sustained 5-min mixed-screen stress, the BSS pool's block-list fragmented until corruption tripped a safety reboot every 90-120 s (heap_wd lands inside `lv_mem_monitor_core → block_size(block=0x90b80c68)` — out-of-range link pointer).
- **Root Cause (the fragmentation class):** LVGL 9.2.2's TLSF tries pools in registration order.  The base 64 KB BSS pool is registered by LVGL init FIRST; the 2 MB PSRAM pool is added by main.c via `lv_mem_add_pool` SECOND.  TLSF returns the first-fitting block from the first pool that has one — so all widget allocations land in BSS until BSS is full, only spilling to PSRAM under pressure.  Sustained widget churn fragments the BSS pool faster than it can be reclaimed.
- **Two obvious alternatives that didn't work:**
  1. **Naive hide/show** (`lv_obj_add_flag(scr_camera, HIDDEN)` + keep widgets resident) — pinned all 30 widgets in BSS → 5-15 KB internal SRAM consumed permanently → SRAM exhaustion watchdog (#184, threshold 20 KB largest block sustained 2 min) tripped FASTER than the original fragmentation.  Tested + reverted.
  2. **Per-screen LVGL display + own pool** — architecturally clean (route camera's allocations to PSRAM via a per-display pool), but LVGL 9.2.2 has no API to redirect allocations per-display.  Widget allocs go through the global TLSF chain regardless of `lv_display_t` ownership.  Would require LVGL upstream changes.  Out of scope for one wave.
- **Fix (the third option, hybrid):** Keep `scr_camera` AND `canvas_buf` resident across hide cycles, but `lv_obj_clean()` the inner widget tree on hide.  Widget pointers are explicitly NULLed (now dangling).  On re-show, `ui_camera_create`'s idempotent fast-path checks `lv_obj_get_child_cnt(scr_camera) > 0` — children gone means rebuild needed; children present means true re-entry, return immediately.  `alloc_canvas_buffer` is now idempotent: same dimensions reuses the buffer (memset zero); different dimensions free + realloc (covers cb_rotate_btn's 0→1→2→3 cycle which swaps canvas_w/h).
- **What this DOES and DOESN'T fix:**
  - DOES: Eliminates 1.8 MB PSRAM alloc + memset on every re-open (~50 ms saved per cycle).  Eliminates `scr_camera` object alloc/free churn.
  - DOES NOT: The widget alloc count per cycle is unchanged (30 destroys + 30 allocs through TLSF).  BSS pool still fragments at the same rate per-cycle.
  - WHY IT WORKS ANYWAY: Live measurement shows 397 s of mixed-screen stress without a reboot (pre-fix: reboot every 90-120 s).  The hypothesis is that the eliminated PSRAM churn + the eliminated screen-wrapper churn both add slack to the per-cycle TLSF op count, pushing fragmentation-corruption out past the 5-min stress envelope.  The real architectural fix (per-screen pool) is still TBD if extended-stress shows the bug returning.
- **Prevention:**
  1. **Three options for screen lifecycle, not two.**  When evaluating "destroy/recreate vs hide/show", remember the hybrid: keep the screen wrapper + heavy buffers resident, `lv_obj_clean` the inner tree.  Combines low-permanent-SRAM-cost (just the wrapper) with bounded-allocation churn savings.  Worked for camera; will likely work for files (next candidate, same architecture).
...
## TT #481 W7-E.4c Wave 7 — Show-path short-circuit hides per-show side effects

- **Symptom:** Reply-context chip ("Replying to Mom (tg)") rendered correctly on the *first* open of the voice overlay but stayed blank if the user armed reply context while the overlay was already up (e.g. mid-conversation → high-pri channel_message → REPLY tap).  No crash, no log, just missing UI.
- **Root cause:** `ui_voice_show()` short-circuits at the top with `if (!s_overlay || s_visible) return;` (correct, prevents double-paint).  But the chip-paint block lived *inside* `ui_voice_show()`, so the second show was a no-op — including the chip refresh.  The arming code in `ui_notification.c::ui_notification_reply_current` correctly called `voice_arm_channel_reply`, but the visual was tied to overlay-creation, not to context-arming.
- **Fix (PR #488):** Extract the chip-paint into a new public `ui_voice_refresh_reply_chip(void)` helper.  `ui_voice_show` calls the helper (first-open behavior unchanged).  `ui_notification_reply_current` also calls the helper right after `voice_arm_channel_reply`, paints on either path.  Idempotent — safe to call before/after overlay built (bails when widget is NULL).
- **Prevention:** When adding **per-event side effects** to overlay show paths, ask: "what if the overlay is already shown?"  If the side-effect should fire on every event regardless of overlay state, extract it into a public refresh helper and call it from BOTH the show-path AND the event-emitter.  Same pattern applies to chat overlay (`ui_chat_show_partial`) and now-card (`ui_home_show_channel_now`) — both already follow this discipline; the W7-E.4c chip was the outlier because it was tied to context-arming instead of a per-message event.

## TT #484 Wave 7 — Debug touch-inject must beat the real digitizer's read loop

- **Symptom:** During e2e harness runs that drive the screen via `POST /touch`, the harness's tap landed at the right pixel for ~30 ms then got immediately overwritten by a stray real-digitizer read (capacitive noise or a finger ghost on the panel).  Net effect: synthetic taps registered as press-then-release-then-press cycles, breaking long-press detection and confusing widget event dispatch.
- **Root cause:** Tab5's LVGL `touch_read_cb` reads two sources: `tab5_debug_touch_override` (the harness inject) and the live ST7123 digitizer.  Both write to the same `lv_indev_data_t` field, and the digitizer poll won the last-write race in the same LVGL tick.
- **Fix (PR #484):** Re-ordered the read so the debug override takes priority — if the override seqlock holds a valid sample within the inject window, the digitizer poll is skipped for that tick.  Window is bounded to `duration_ms + 50 ms` slack so a finished inject never blocks real touch indefinitely.
- **Prevention:** Any two-source input read needs an **explicit priority + handoff window**.  "Last writer wins" is a bug, not a design.  When debug + real both feed the same field, the debug write should win during its declared lifetime; the real source resumes at the next tick after the lifetime expires.

## W7-E Wave 7 — Ring-buffer patterns for notification surfaces (dedupe + snooze)

- **Why two rings?** `channel_message` frames have two orthogonal concerns: (a) dedupe identical IDs that arrive twice (Dragon retry / WS reconnect replay), and (b) snooze on user demand for 15 min.  Conflating them into one ring + flag bits is tempting but mixes lifecycles — dedupe entries must live ~1 hour to absorb retry storms, snooze entries must persist until fired *or* dismissed.  Separate rings keep each invariant clean.
- **Sizing:** 32-entry dedupe ring (BSS-resident, ~2 KB) covers a busy channel-message hour with headroom; 8-entry snooze ring covers human-scale "I'll deal with these later" depth.  Both keyed on `message_id`.
- **Walker pattern:** Snooze ring is checked by a 60 s `lv_timer` (`snooze_walk_cb`).  Walks in **reverse** so it can remove entries without re-indexing.  Re-fires the message through the normal `ui_notification_show` path so high-pri snoozes pop back as now-cards naturally.  Refire clears the dedupe slot so the W7-E.3 guard doesn't drop it.
- **Quiet-hours interaction (W7-E.6):** Walker gates on `quiet_now()` at entry — ring left untouched during quiet hours, processed on next 60 s tick after quiet ends.  No fire-during-sleep, no lost messages.
- **Prevention:** When you find yourself adding flag bits to a ring entry ("snoozed", "delivered", "muted") to track lifecycles, consider whether the entry actually belongs in a different ring.  Two rings with one invariant each beat one ring with three invariants.

## W7-E.4b Wave 7 — Armed-context handoff between UI tap (LVGL task) and STT-complete (WS task)

- **Pattern:** "When user taps REPLY on a now-card, the next utterance the user dictates should send as `channel_reply` instead of normal LLM dispatch."  Two tasks involved: LVGL (REPLY tap) and WS (STT-complete frame).
...
## TT #621 — Tab5↔K144 USB transport via userspace functionfs bridges (2026-05-21)

**Date:** 2026-05-21
**Symptom:** TT #620's cdc_acm path enumerated cleanly + INTR endpoint worked, but every byte Tab5 wrote to bulk OUT was silently dropped by K144 before reaching `/dev/ttyGS0`.  Dev-box probes against the same K144 worked fine, so the bug was specific to Tab5-as-USB-host vs K144's `f_acm` tty layer.  Every documented software fix (CLEAR_FEATURE on both endpoints, SET_INTERFACE, DTR transition, line coding, vendored cdc_acm_host v2.4, K144 firmware 1.6) failed.
**Root Cause:** Specific (still uncharacterised) interaction between Tab5's `usb_host` driver and K144's kernel `f_acm` bulk-OUT-to-tty delivery path.  Visible only when Tab5 is the host; not when the dev box is the host.
**Fix:** Bypass `f_acm` entirely with userspace `functionfs` bridges on K144.  ADB already proves the pattern works (the kernel's `g_ffs` exposes raw bulk endpoints that `adbd` claims from userspace).  We added two more `ffs.*` functions to K144's composite gadget (`ffs.control` for the StackFlow JSON control plane, `ffs.video` for YOLO inference frames), with a small C daemon per function that relays bytes between the bulk endpoints and a TCP socket on `127.0.0.1:10001` (where StackFlow's `llm-sys` already listens).  On Tab5, **one** USB host client claims both Tinker interfaces by walking the config descriptor and matching unique subclass values (0x44 control, 0x43 video — ADB's 0x42 stays untouched).  Per-channel: recursive mutex, PSRAM stream buffer, persistent OUT transfer + done semaphore (allocated once at claim, reused per send), zero-copy borrow/commit API on video so callers can encode base64 directly into the USB DMA buffer.
**Prevention:**
- **Don't free in-flight USB transfers.** ESP-IDF's `usb_host_transfer_free` explicitly forbids it; doing so on send timeout caused use-after-free panics under bursty load.  Use pre-allocated persistent transfers + endpoint halt+flush+clear on timeout, never free.
- **Don't run two USB host clients claiming the same device.**  Each client's per-device tracking eats ~25 KB of internal SRAM; on Tab5 (already at ~56 KB largest free) a second client pushed largest free to 0 KB, panicking on the next big alloc.  One client + multiple interface claims keeps overhead flat.
- **PSRAM-back USB DMA buffers.**  `CONFIG_USB_HOST_DWC_DMA_CAP_MEMORY_IN_PSRAM=y` is critical for any non-trivial USB host workload on ESP32-P4; default is internal SRAM which gets exhausted fast.
- **Don't hardcode interface numbers across a gadget composition.**  K144's kernel reshuffles `bInterfaceNumber` as functions are added/removed; in W6 our hardcoded `interface 2` ended up pointing at CDC-ACM Data after we composed `ffs.video` in.  Identify channels by unique subclass instead, then discover the interface + endpoint addresses from the descriptor.
- **Subclass collisions on vendor class.**  ADB's descriptor uses class=0xff/sub=0x42/proto=0x01.  Any new vendor-class function needs a distinct subclass or its claim will steal ADB's slot in a descriptor walk (W6 first cut hit this).
- **HTTP body buffers go to PSRAM.** `malloc(content_len)` in an `httpd` handler will satisfy from internal SRAM; a 30 KB JPEG body per call exhausts it within ~3 calls and starves Wi-Fi DMA.  Use `heap_caps_malloc(len, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT)`.
- **Per-call workspace mallocs in a hot loop kill PSRAM bandwidth.**  voice_yolo originally `heap_caps_malloc`d a b64 buffer + JSON wrapper + drain buffer per inference.  At >1 fps that's enough PSRAM allocator churn to flap Wi-Fi.  Pre-allocate workspaces at init, write the base64 directly into the USB TX buffer via the borrow API.
- **Kernel doesn't like ungraceful `g_ffs` daemon exit.**  Killing the daemon (SIGKILL) while the gadget is UDC-bound takes the whole gadget down; the host sees disconnect and the K144 needs a power-cycle.  Either SIGTERM (let the daemon's signal handler set its run flag and exit cleanly) or use `mv` to atomically swap the binary and `reboot` K144 to respawn from rc.local.
- **Glibc `htole32` isn't a compile-time constant.**  Can't use it in C static initialisers.  On a known-LE target (aarch64 K144) just define identity `LE32(x) (x)` / `LE16(x) (x)` macros for the descriptor blobs.
- **Wave-by-wave verification.**  Each W5-W9 wave shipped with a live hardware check.  W5: dev-box probe + Tab5 hwinfo round-trip.  W6: dog/cat JPEG → detection JSON.  W7: `/tinkeron/extpcm` showing `frames_pumped` climbing.  W8: full chain to `wakeword.task start` in obs.  W9: cold boot picks USB without manual intervention.

---

## TT #621 follow-up — Navigation centralisation + voice cancel semantics (2026-05-21)

**Date:** 2026-05-21
**Symptom:** Two reports during the post-USB-pivot sweep.  (a) After saying "Hey Tinker" → live voice turn → tapping the X on the voice overlay or the back arrow on the chat header took the user back to home, but a stale wakeword "LISTENING" tail could keep feeding K144 ASR partials and silently fire a DICTATION_FINAL after the overlay was already closed — voice felt "uncancellable".  (b) Every screen had its own ad-hoc nav handler; 9 separate entry points, zero voice-state checks, and no consistent debounce.  Nav-during-voice was a coin flip between "voice keeps running underneath the new screen" and "voice cancels but the screen change races the cancel ack".
**Root Cause:** Two parallel state machines (Dragon-side `voice.c` state + wakeword's own `s_state` in `voice_wakeword.c`) with no shared cancellation — `voice_cancel()` reset Dragon-side state but never touched the wakeword listener.  Combined with un-centralised nav: each screen made its own `lv_screen_load`/`tab5_debug_set_nav_target` call with no voice cleanup hook.
**Fix:** 5-wave sweep shipped as PRs #624/#625/#627/#630/#632.
- **Wave 0 (foundation):** New `main/ui_nav.{c,h}` module + `tab5_nav_to(target, flags)` is now the single nav chokepoint.  Owns debounce (`ui_tap_gate("nav:<target>", 300)`), voice cancel (if not IDLE/READY → `voice_cancel()` + 200 ms settle), then dispatches to the existing async-LVGL nav.  All 11 prior callsites migrated; `tab5_debug_set_nav_target` kept as a thin shim.
- **Wave A (user-visible bugs):** `voice_cancel()` now also calls `voice_wakeword_force_dictation_stop()` + `voice_ext_pcm_stream_set_paused(true)` with a 1500 ms unpause timer (R2 — closes the stale wakeword tail).  `ui_mode_sheet::persist_and_notify_dragon` auto-cancels in-flight turn before applying a vmode change + toasts "Switched mode — current turn stopped" (R6).  `voice_start_dictation` refuses with `ESP_ERR_INVALID_STATE` + toast "Voice busy — wait for current turn" when mic is already in use (R7).
- **Wave B (races):** `WAKE_REARM_GRACE_MS` bumped 1000 → 1500 ms (R3).  `voice_usb_ffs.c` tracks `s_had_prior_connect` on DEV_GONE + re-runs `voice_xport_init` via `xport_reswitch_job` worker after re-claim (R9).
- **Wave C (edge cases + obs):** Channel-reply context gains 30 s TTL stamped at arm + expired on read (R14).  `voice_m5_llm`'s `sys.reset`/`sys.reboot` now clear `s_tts_work_id` alongside `s_setup_work_id` (R12).  New `voice_yolo_invalidate()` API + hook from `m5.reset:recovered` so yolo cached handle doesn't survive a K144 daemon restart (R12b).  Notification z-order re-asserted on every voice state edge so the X close button always wins (R8a).  REPLY tap refuses + toasts "Wait for current reply" when voice state == PROCESSING (R8b).  9 new obs fields on `/tinkeron/extpcm` + `/m5` (wakeword_state, ms_since_busy, dictation_final_count, channel_reply armed/age/ttl_remaining, work_ids{llm,tts,yolo_ready}, ms_since_last_reset).
- **Wave D:** New `story_wave_c_stability` e2e scenario (9 steps, 23 s, all pass on Tab5 192.168.1.90) exercises the new endpoints + nav cascade + voice.cancel obs event + K144 reset cycle.
**Prevention:**
1. **One nav chokepoint per app.**  Any non-trivial UI hits at least N×N nav paths (N screens × N entry points).  Centralising once cuts the matrix to 1×N — every screen calls the chokepoint and the chokepoint owns lifecycle (debounce, voice cancel, overlay hide, screen swap, foreground button visibility).  Resist the urge to inline `lv_screen_load` in handlers; route through the chokepoint even for "simple" back paths.
2. **State machines that are logically coupled must be coupled mechanically.**  Wakeword state and Dragon voice state were two halves of "voice listening".  Treating them as independent let the wakeword tail outlive the user's cancel intent.  Either fuse them into one machine or add a contract that any transition out of LISTENING/PROCESSING/SPEAKING resets BOTH halves.  We chose the latter — `voice_cancel` now resets both — because the wakeword listener has its own valid IDLE/LISTENING states for between-turn use.
3. **TTL on armed UI contexts.**  Any "the next user action means X" state (channel reply, snooze, undo) should auto-expire after N seconds.  Without a TTL the context leaks across turns + screens; with one the user gets predictable "stale context cleared" behavior.  Implement expiry-on-read inside the consume/peek API so callers can't accidentally use a stale snapshot.
