...
| W7-F.5 (queued) | OpenClaw config | — | Operator-level enable a channel plugin in OpenClaw's `~/.openclaw/openclaw.json` (Telegram bot token, WhatsApp pairing, etc.) so the gateway's `send` method round-trips with a real `platform_message_id`.  Outside the connector contract — just `openclaw config set channels.telegram.token=...` + plugin start.  When green, Tab5 ack will surface `ok=true platform_message_id=<real-id>` end-to-end. |
