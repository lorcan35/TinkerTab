...
| W7-F.4 (queued) | TinkerBox | — | Device-identity HMAC handshake to pass the gateway's `!isControlUi → clearUnboundScopes` rule for device-less backends (`openclaw/src/gateway/server/ws-connection/message-handler.ts:L510-512`).  Required to obtain `operator.write` scope without impersonating Control UI.  Scope: ed25519 keypair generation + persistence + `buildDeviceAuthPayloadV3({deviceId, clientId, clientMode, role, scopes, signedAtMs, token, nonce, platform, deviceFamily})` + `signDevicePayload` per `openclaw/src/gateway/client.ts:418-441`.  ~150-200 LOC + tests.  Unlocks real platform_message_id round-trips through configured channel plugins (Telegram, WhatsApp, etc.). |
