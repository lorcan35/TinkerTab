...
**Wave 7 progressing** — W7-A + W7-A.b + W7-B + W7-A.2 all shipped 2026-05-12.  Mode 3 now (1) surfaces real-time gateway tool calls to Tab5, (2) flips them to "done" on completion via synthesized tool_result events, (3) records both phases into the cross-session agent_log feed, (4) exposes a skill catalog for Tab5's Agents overlay.  Full visibility round-trip on the mode-3 agentic loop without touching the gateway.  Next: W7-C (memory bridge) or W7-B.2 (dynamic skill discovery via WS-RPC).
