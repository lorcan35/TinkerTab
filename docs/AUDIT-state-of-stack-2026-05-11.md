...
**Wave 7 progressing — 5 slices shipped 2026-05-12** — Mode 3 now (1) surfaces real-time gateway tool calls to Tab5, (2) flips them to "done" on completion via synthesized tool_result events, (3) records both phases into the cross-session agent_log feed, (4) exposes a skill catalog for Tab5's Agents overlay, (5) **Tab5 firmware live-verified fetching the catalog on hardware**.  Full Tab5↔Dragon mode-3 visibility loop closed today.  Next: W7-B.3 LVGL render of the catalog in Agents overlay, then W7-C (memory bridge) or W7-B.2 (dynamic skill discovery via WS-RPC).

**Remaining roadmap:** W7-B.3 Tab5 LVGL render · W7-C memory bridge · W7-B.2 dynamic skill discovery (WS-RPC) · W7-0 notification design → W7-D/E/F/G · W10-B docs-site decision · W10-D fork scope-down (gated on W7 outcome).
