...
### Phase 6a — adaptive UART baud negotiation — DONE 2026-04-29

**File:** `bsp/tab5/uart_port_c.{c,h}` + `main/voice_m5_llm.{c,h}`.  Added `voice_m5_llm_set_baud()`, `voice_m5_llm_recover_baud()`, `voice_m5_llm_get_baud()`.  Adaptive contract: send `sys.uartsetup` at OLD baud → ACK → switch Tab5 → 5x retry ping (200 ms settle) at NEW baud, revert if any fail.

**Bench (Tab5 → Mate carrier → K144), 30 pings each baud:**

| Baud      | Switch | Pings | Avg latency |
|-----------|--------|-------|-------------|
| 115200    | OK     | 30/30 | 124 ms      |
| 230400    | OK     | 30/30 | 116 ms      |
| 460800    | OK     | 30/30 | 111 ms      |
| 921600    | OK     | 30/30 | 110 ms      |
| **1 000 000** | **OK** | **30/30** | **109 ms** |
| **1 500 000** | **OK** | **30/30** | **109 ms** |
| → 115200  | OK     |  5/5  | 125 ms      |
...
**Files:** `main/voice_m5_llm.{c,h}` — added `voice_m5_llm_tts(text, pcm_out, max_samples, &out_samples, timeout_s)`.  Reuses cached TTS work_id with same auto-recovery on `-4`/`-25` errors as the LLM path.  Decodes base64 via `mbedtls_base64_decode`.  Returns 16 kHz mono `int16_t` PCM (caller upsamples 1:3 to feed Tab5's 48 kHz speaker pipeline).  REPL helper `m5tts <text>` exercises full chain end-to-end.

**Live verification (Tab5 → Mate carrier → K144 → Tab5 speaker):**
```
> m5tts hello world
I (47422) voice_m5_llm: tts.setup ok — work_id=tts.1037 model=single_speaker_english_fast
I (52274) voice_m5_llm: tts: 15360 samples @ 16 kHz mono (960 ms speech)
[m5tts] ESP_OK in 5699ms (15360 samples = 960 ms speech)
[m5tts] played 46080 samples @ 48k
```

960 ms of synthesized speech generated by the K144's `single_speaker_english_fast` model, sent over UART, decoded on Tab5, upsampled to 48 kHz, played through Tab5's actual speaker.  End-to-end latency ~5.7 sec for a 1 sec utterance, dominated by setup + 30 KB base64 transfer at 115200 baud.  Negotiating to 1 Mbps (Phase 6a) drops the transfer to ~30 ms, bringing total turnaround to ~1.5-2 sec.

**Three quirks documented in LEARNINGS:**
- `tts.base64.wav` returns headerless 16 kHz mono PCM, NOT a RIFF WAV.
- TTS responses are single frames up to 30+ KB; rx buffer was bumped from 4 KB to 384 KB.
- Stale TTS work_id after K144 service restart returns error `-4`; auto-recovery clears the cache (same pattern as LLM).
