...
{
    /*
     * TODO: include the real config blob:
     *   #include "bmi270_config.h"
     *   static const uint8_t *cfg = bmi270_config_file;
     *   static const size_t   cfg_len = sizeof(bmi270_config_file);
     */
    const uint8_t *cfg     = NULL;
    const size_t   cfg_len = 0;

    if (cfg == NULL || cfg_len == 0) {
        ESP_LOGW(TAG, "BMI270 config blob not linked — skipping firmware upload");
        ESP_LOGW(TAG, "Accel/gyro may still work in basic mode on some revisions");
    ...
    /* Burst-write config in 256-byte chunks */
