...
static uint8_t * sdio_rx_get_buffer(uint32_t len)
{
	...
	if (len > double_buf.buffer[index].buf_size) {
		/* Try to allocate the full requested size.
		 * Keep old buffer as fallback — don't free before successful alloc.
		 * This prevents OOM from leaving us with no buffer at all (#18). */
		uint8_t *new_buf = (uint8_t *)MEM_ALLOC(len);
		if (!new_buf) {
			/* Retry once after brief delay to let other tasks free memory */
		...
		if (new_buf) {
			...
			*buf = new_buf;
			double_buf.buffer[index].buf_size = len;
			ESP_LOGD(TAG, "buf %d size: %ld", index, len);
		} else if (*buf && double_buf.buffer[index].buf_size > 0) {
			/* Keep existing smaller buffer — read will be truncated
			 * but we still read from slave to prevent SDIO desync */
			ESP_LOGW(TAG, "SDIO RX alloc failed (%ld), using existing %ld buf",
				len, double_buf.buffer[index].buf_size);
		} else {
			ESP_LOGE(TAG, "SDIO RX buf alloc FAILED (%ld bytes) — no fallback", len);
...
static esp_err_t sdio_push_data_to_queue(uint8_t * buf, uint32_t buf_len)
{
	...
	do {
		...
		if (!pkt_rxbuff) {
			ESP_LOGE(TAG, "sdio_buffer_alloc failed — dropping remaining stream packets");
			return ESP_FAIL;
		}
...
static void sdio_read_task(void const* pvParameters)
{
	...
	for (;;) {
		...
		if (!rxbuff) {
			ESP_LOGE(TAG, "sdio_rx_get_buffer NULL — OOM, skipping %ld bytes (#18)", len_from_slave);
			/* Advance byte counter to keep host/slave in sync.
			 * We cannot safely drain here — stack buffers aren't DMA-aligned
			 * on P4 and cause cache corruption. The slave will stall briefly
			 * but recover on next successful alloc or reconnect. */
			sdio_rx_byte_count += len_from_slave;
			sdio_rx_byte_count = sdio_rx_byte_count % ESP_RX_BYTE_MAX;
			SDIO_DRV_UNLOCK();
			vTaskDelay(pdMS_TO_TICKS(50));  /* Back off to let memory free up */
			continue;
		}

		/* Clamp read to buffer capacity — if alloc failed and we kept the
		 * old smaller buffer, reading more would overflow it (#18). */
		{
			uint32_t buf_cap = double_buf.buffer[double_buf.write_index].buf_size;
			data_left = (len_from_slave > buf_cap) ? buf_cap : len_from_slave;
		}
		pos = rxbuff;
		...
		if (double_buf.read_index < 0) {
			...
			/* Use actual bytes read (may be < len_from_slave if buffer was capped) */
			uint32_t buf_cap2 = double_buf.buffer[double_buf.write_index].buf_size;
			double_buf.read_data_len = (len_from_slave > buf_cap2) ? buf_cap2 : len_from_slave;
