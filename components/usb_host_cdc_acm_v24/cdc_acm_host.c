...
static esp_err_t cdc_acm_start(cdc_dev_t *cdc_dev, cdc_acm_host_dev_callback_t event_cb, cdc_acm_data_callback_t in_cb, void *user_arg)
{
    ...
    ESP_LOGI(TAG, "TT#621: Claiming data interface %d alt=%d",
             (int)cdc_dev->data.intf_desc->bInterfaceNumber,
             (int)cdc_dev->data.intf_desc->bAlternateSetting);
    ESP_GOTO_ON_ERROR(
    ...
    /* TT#621 EXTRA FIX: CLEAR_FEATURE(ENDPOINT_HALT) on both bulk
     * endpoints — resets USB DATA0/DATA1 toggle on both host and
     * device.  Without this, Tab5 sends DATA0 but K144's f_acm may
     * have stale DATA1 expectation from a previous session and
     * silently drops the packet as a "duplicate" while still ACKing
     * it at the wire level.  This ensures clean toggle alignment.
     *
     * USB std request: bmRequestType=0x02 (host→device, std,
     * endpoint), bRequest=0x01 (CLEAR_FEATURE), wValue=0
     * (ENDPOINT_HALT), wIndex=ep_addr.
     */
    {
        ESP_LOGI(TAG, "TT#621: CLEAR_FEATURE(ENDPOINT_HALT) on BULK OUT 0x%02X",
                 (unsigned)cdc_dev->data.out_xfer->bEndpointAddress);
        esp_err_t ch_out = cdc_acm_host_send_custom_request(
            (cdc_acm_dev_hdl_t)cdc_dev,
            /*bmRequestType=*/0x02,
            /*bRequest=*/0x01,
            /*wValue=*/0,
            /*wIndex=*/cdc_dev->data.out_xfer->bEndpointAddress,
            /*wLength=*/0,
            /*data=*/NULL);
        ESP_LOGI(TAG, "TT#621: CLEAR_HALT OUT → %s", esp_err_to_name(ch_out));
        if (cdc_dev->data.in_xfer) {
            ESP_LOGI(TAG, "TT#621: CLEAR_FEATURE(ENDPOINT_HALT) on BULK IN 0x%02X",
                     (unsigned)cdc_dev->data.in_xfer->bEndpointAddress);
            esp_err_t ch_in = cdc_acm_host_send_custom_request(
                (cdc_acm_dev_hdl_t)cdc_dev,
                /*bmRequestType=*/0x02,
                /*bRequest=*/0x01,
                /*wValue=*/0,
                /*wIndex=*/cdc_dev->data.in_xfer->bEndpointAddress,
                /*wLength=*/0,
                /*data=*/NULL);
            ESP_LOGI(TAG, "TT#621: CLEAR_HALT IN → %s", esp_err_to_name(ch_in));
        }
    }

    /* TT#621 KEY FIX: explicit SET_INTERFACE(alt=0) on the data
     * interface BEFORE submitting any URBs.  Linux f_acm's set_alt
     * callback calls gserial_connect() which wires the bulk
     * endpoints to /dev/ttyGSn's tty buffer.  Without this, OUT
     * bytes USB-ACK but never appear in /dev/ttyGSn reads.
     *
     * usb_host_interface_claim only sends SET_INTERFACE on the wire
     * if alt > 0.  For alt=0 (which is our case), the wire request
     * is skipped — but the gadget's set_alt is also never invoked.
     *
     * We use a custom control transfer to force SET_INTERFACE alt=0
     * on the wire.  USB std request: type=0x01 (host→device, std,
     * recip=interface), bRequest=0x0B (SET_INTERFACE), wValue=alt=0,
     * wIndex=interface_number, wLength=0.
     */
    {
        ESP_LOGI(TAG, "TT#621: forcing SET_INTERFACE(alt=0) on data intf %d",
                 (int)cdc_dev->data.intf_desc->bInterfaceNumber);
        esp_err_t si = cdc_acm_host_send_custom_request(
            (cdc_acm_dev_hdl_t)cdc_dev,
            /*bmRequestType=*/0x01,
            /*bRequest=*/0x0B,
            /*wValue=*/0,
            /*wIndex=*/cdc_dev->data.intf_desc->bInterfaceNumber,
            /*wLength=*/0,
            /*data=*/NULL);
        ESP_LOGI(TAG, "TT#621: SET_INTERFACE result: %s", esp_err_to_name(si));
    }

    if (cdc_dev->data.in_xfer) {
        ESP_LOGI(TAG, "TT#621: submitting initial BULK IN poll on ep=0x%02X",
                 (unsigned)cdc_dev->data.in_xfer->bEndpointAddress);
...
static void in_xfer_cb(usb_transfer_t *transfer)
{
    /* TT#621: promote to INFO so we see every bulk-IN completion. */
    ESP_LOGI(TAG, "in xfer cb: status=%d actual=%d/%d ep=0x%02X",
             (int)transfer->status, (int)transfer->actual_num_bytes,
             (int)transfer->num_bytes, (unsigned)transfer->bEndpointAddress);
    cdc_dev_t *cdc_dev = (cdc_dev_t *)transfer->context;

    if (!cdc_acm_is_transfer_completed(transfer)) {
        ESP_LOGW(TAG, "in xfer cb: NOT completed (status=%d) — clearing halt + resubmit",
                 (int)transfer->status);
        if (transfer->status != USB_TRANSFER_STATUS_NO_DEVICE &&
            ...
            cdc_acm_reset_in_transfer(cdc_dev);
            esp_err_t re = usb_host_transfer_submit(cdc_dev->data.in_xfer);
            ESP_LOGI(TAG, "in xfer cb: resubmit → %s", esp_err_to_name(re));
        }
        return;
    }
