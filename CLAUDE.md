...
- **Phase 6 status:** UART baud verified clean to 1.5 Mbps (Phase 6a); K144 TTS verified speaking through Tab5's speaker (Phase 6c).  ASR (mic→K144) deferred — K144's audio architecture is subscription-only (LEARNINGS: "K144 ASR is subscription-only — no push-PCM-via-JSON path exists").
