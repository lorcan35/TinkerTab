...
- **Voice loop + WS health audit — CLOSED 2026-05-25** → 6 PRs squash-merged on main from a live cross-stack audit of the Tab5↔Dragon↔K144 piping.  Triggered by the user-reported "X-button-cancel re-opens listening" loop; expanded into a piping audit that found 4 more independent bugs.  Closed PRs (in merge order, all squashed): **#693** Wave A — revert PR #691's permanent wakeword-disarm + add `voice_wakeword_post_cancel_suppress_ms(3000)` API called by `voice_cancel` + bump ext_pcm pump pause 1500→3000 ms + anchor wake matcher on freshly-arrived ASR delta (first-half + 4 char slack) instead of accumulated 96-byte sliding window (closes #692).  **#699** Drop consecutive identical non-finish ASR partials at front of `asr_partial_cb` — K144 sherpa-ncnn rolling decoder re-emits same partial 10×/sec (live-observed) and every duplicate ran 7× istrstr + transcript ring critical section + LOGI ×2 for zero new signal.  Also demoted the two chatty asr-frame/asr-delta `ESP_LOGI` to `ESP_LOGD` (closes #698).  **#697** WS-RX transient branch in `voice_ws_proto.c::voice_ws_proto_handle_text` now snaps `voice_state` back to READY/IDLE — pre-fix a transient `stt_empty`/`pipeline_failed` error left Tab5 stuck in PROCESSING forever (mic tap + wake refused until reboot).  Fatal branch already did the snap-back; transient was missing parity (closes #696).  **#695** Top-right reboot button on home — tap = "Hold to reboot" safety toast, long-press = "Rebooting…" + esp_restart() via 600 ms esp_timer one-shot so the toast paints first (closes #694).  **#701** `vision_service::detections_total` counts FILTERED (person-class) detections only — was counting raw YOLO output (chairs/cups/books) while `last_class` etc. only updated for the filter, making `/vision/state` show "detections_total=3 + last_class=''" (closes #700).  **#703** `ui_notes_unprocessed_count()` drops the FAILED branch — counter was counting FAILED-with-audio notes that `transcription_queue_task` deliberately refuses to retry, making `/logs/tail` log "Transcription queue: 1 unprocessed" every 15 s forever (closes #702).  Live-verified on Tab5 192.168.1.90 post-flash: state=READY, wakeword armed, asr.1005 binding clean, 0 panic/abort/leak lines, internal-SRAM `largest=62 KB @ 60s` (UP from 52 KB @ 22min pre-fix — dedup + Wave A actually reduced steady-state heap pressure).  Audit also confirmed clean: voice_set_state mutex, settings NVS mutex, cJSON Parse/Delete balance in voice_ws_proto.c, playback ring buffer mutex.  Out-of-scope follow-ups noted but not blocked: 24 h heap soak (catch slow leaks the 40-min window can't), WS reconnect under simulated Dragon restart (verify 180 s pong timeout doesn't mask real failures — currently deliberate per `WS_CLIENT_PONG_SEC` comment in voice.c:134), OPUS encoder revival (gated off via voice_codec.h, root-cause SILK NSQ ESP32-P4 crash pending TT #264), channel_reply TTL expiry path live exercise.

- **TT #621 follow-up — Stability + navigation overhaul — CLOSED 2026-05-21** → 5 wave PRs squashed to main (#624 / #625 / #627 / #630 + #632) closing #622 → #629.  **Wave 0** introduced `main/ui_nav.{c,h}` as the single navigation chokepoint — every screen swap now flows through `tab5_nav_to(target, flags)` which (a) debounces via `ui_tap_gate("nav:<target>", 300)`, (b) calls `voice_cancel()` if voice is mid-turn (with a 200 ms settle window before the screen swap), (c) dispatches via the existing async-LVGL nav path.  All 11 prior nav callsites (`ui_home`, `ui_settings`, `ui_camera`, `ui_notes`, `ui_files`, `ui_chat`, `ui_wifi`, `ui_sessions`, `ui_chrome`, `chat_header`, `debug_server_nav`) now route through it.  **Wave A** closed three user-visible bugs: voice_cancel now resets the wakeword state machine + pauses ext_pcm pump for 1500 ms (R2), vmode change mid-turn auto-cancels with toast (R6), and voice_start_dictation refuses with toast when mic already busy (R7).  **Wave B** bumped wake re-arm grace 1000→1500 ms (R3) and added USB replug auto-restore via `s_had_prior_connect` + `xport_reswitch_job` (R9).  **Wave C** added channel_reply 30 s TTL with expire-on-read (R14), cached work_id invalidation on K144 reset for TTS + yolo (R12 + R12b — voice_yolo gained a `voice_yolo_invalidate()` API hooked from `m5.reset:recovered`), notification z-order re-assertion on voice state edges (R8a), REPLY-mid-PROCESSING gate with toast (R8b), and 9 new obs fields surfaced via `/tinkeron/extpcm` + `/m5` (wakeword_state, ms_since_busy, dictation_final_count, channel_reply armed/age/ttl_remaining, work_ids{llm,tts,yolo_ready}, ms_since_last_reset).  **Wave D** added `story_wave_c_stability` to the e2e harness (9 steps, ~23 s, all pass).  Live-verified on Tab5 192.168.1.90: `/m5/reset` → work_ids.llm cleared from `llm.1003` → `""` and `ms_since_last_reset` populated post-recovery (3.6 s → climbing).  Residual ops: 12-hour soak test of the modified `story_stress` loop tracked separately.

- **TT #621 — Tab5↔K144 USB transport pivot via functionfs — DONE 2026-05-21** → [`docs/internal/PLAN-usb-cdc-pivot.md`](docs/internal/PLAN-usb-cdc-pivot.md) (TT #621 section) + LEARNINGS "TT #621".
  M5-Bus UART at 1.5 Mbps is replaced by USB host (Tab5 USB-A → K144 top USB-C) running two userspace `functionfs` bridges on K144 (`ffs.control` subclass 0x44 for StackFlow JSON + `ffs.video` subclass 0x43 for YOLO frames).  A single Tab5 USB host client (`main/voice_usb_ffs.{c,h}`) claims both interfaces by subclass discovery (kernel reshuffles `bInterfaceNumber` per gadget composition), pre-allocates a persistent OUT transfer + done semaphore per channel (eliminates use-after-free panic_aborts that hit when freeing in-flight transfers on send timeout), and exposes a zero-copy borrow/commit TX API on the video channel so `voice_yolo` encodes base64 directly into the USB DMA buffer (halves PSRAM bandwidth churn during inference bursts).  Default NVS `xport` flipped from 0 (UART) → 2 (USB); `voice_xport_init` still falls back to UART if USB doesn't enumerate within the boot window.  Live-verified: cold boot → `xport=usb_ffs`, `failover_state=ready`, ext_pcm `frames_pumped` climbing ~10 fps, `wakeword_active=true`, `asr.1001` assigned; `POST /yolo/infer` with a 320×320 JPEG returns detection JSON in ~330 ms.  Known follow-up: sustained YOLO bursts (3+ calls in <1s) cause Wi-Fi DMA contention → WS reconnect (recoverable, no panic).  Set `CONFIG_USB_HOST_DWC_DMA_CAP_MEMORY_IN_PSRAM=y` in sdkconfig.defaults is critical — internal SRAM was already at 1KB largest after a dual-client attempt; PSRAM-backing USB DMA brought it back to 24KB+.

- **TinkerON wakeword via Tab5 mic — SEAMLESS + SELF-HEALING 2026-05-20** → merged `ef10184` on main (closes TT [#131](https://github.com/lorcan35/TinkerTab/issues/131)).  Tab5 mic → ext_pcm pump → K144 `llm-asr` over M5-Bus UART at 1.5 Mbps → multi-pattern matcher catches "Hey Tinker" → orb-tap voice turn.  **Boot auto-arms** the entire chain (auto-detect K144 baud, asr.setup at 115200 for clean handshake, bump to 1.5 M for pump throughput).  **No manual commands** required after restart.  **K144-ASR-cycling watchdog** (PSRAM stack, polls every 10 s) auto-recovers when K144's daemon cycles; escalates to `sys.reboot` after 2 failed `sys.reset` attempts.  **Stability foundation**: PSRAM-backed task stacks killed the `heap_wd: sram_exhausted` panic-reboot loop (internal SRAM largest-free 56 → 80 KB stable).  **Wake suppressed** during full voice turn (LISTENING + PROCESSING + RECONNECTING + SPEAKING) + 1 s post-turn grace prevents self-wake from Tinker's own TTS reply.  **Multi-pattern matcher** accepts `{thinker, hicker, tinker, hick, hanker}` to cover K144's sherpa-ncnn streaming-zipformer-20M variable rendering of "Hey Tinker".  Live-verified single fire on real speech + full voice turn cycle.  See memory file `project_ext_pcm_stability_fix_2026_05_20.md` for the full architecture audit + recovery recipe + known operational quirks (ES7210 codec drift, UART framing errors at 1.5 M tolerated by JSON parser).

- **TinkerON (K144) always-on wakeword (K144 own mic) — LIVE 2026-05-18** → [`docs/internal/PLAN-wakeword.md`](docs/internal/PLAN-wakeword.md), branch `feat/wakeword`, PR [#576](https://github.com/lorcan35/TinkerTab/pull/576), closes TT [#575](https://github.com/lorcan35/TinkerTab/issues/575).
  Said "Hey Tinker — what time is it?" → Tab5 mic opened → Dragon STT → LLM → Kokoro TTS spoke the time back, hands-free.  Verified live on Tab5 192.168.1.90 (commit `b614a23`).  Wake matcher accepts BOTH `hey tinker` AND `hey thinker` (K144's sherpa-ncnn ASR substitutes T → Th consistently).  WAKE → `voice_start_listening()` (regular orb-tap path → respects per-connection voice_mode routing).  Three K144 stability fixes also landed: `voice_m5_llm_release()` after warmup frees the NPU slot for ASR (fixes `err=-21 task full`); `voice_wakeword_stop()` before `sys.reset` prevents the post-reset re-arm from silently returning `INVALID_STATE`; **NO extra USB-C needed to power TinkerON** — per the M5Stack docs, the Module13.2 LLM Mate "achieves stacked power supply with Module LLM via the M5-Bus interface" and the K144 draws only ~1.5W at full load.  Tab5's own USB-C powers the whole stack via M5-Bus.  The Mate's USB-C is CH340N serial log out (debug only); the K144's top USB-C (M140 port) is USB OTG / Axera ADB (useful when directly debugging the K144 Linux from a dev host).  Both can be unplugged in normal operation.  The K144 wedging we hit during bring-up was SOFTWARE state (stale daemon units, NPU task slot collisions), not power — addressed by the `voice_m5_llm_release()` + `voice_wakeword_stop()` ordering fixes.  Open items (next-up): self-wake suppression during TTS playback, top-bar "TinkerON armed" status chip, NVS `tinkeron_armed` toggle, Settings UI for arm/disarm + reset, barge-in interrupt mid-TTS.

- **UI/UX hardening — CLOSED** → [`docs/internal/AUDIT-ui-ux-2026-04-29.md`](docs/internal/AUDIT-ui-ux-2026-04-29.md) + [`docs/internal/PLAN-ui-ux-hardening.md`](docs/internal/PLAN-ui-ux-hardening.md) + LEARNINGS "TT #328 UI/UX hardening".
...
- **Wave 7 (TinkerClaw / channel messaging) — CLOSED** → [`docs/internal/AUDIT-state-of-stack-2026-05-11.md`](docs/internal/AUDIT-state-of-stack-2026-05-11.md) + [`docs/internal/PLAN-agent-mode-notification-surface.md`](docs/internal/PLAN-agent-mode-notification-surface.md).
...
- Plan: [`docs/internal/PLAN-grove.md`](docs/internal/PLAN-grove.md) · Tracking: [#316](https://github.com/lorcan35/TinkerTab/issues/316)
...
- Plan: [`docs/internal/PLAN-m5-llm-module.md`](docs/internal/PLAN-m5-llm-module.md) · Tracking: [#317](https://github.com/lorcan35/TinkerTab/issues/317)
...
- **Wave 1-7 chain hardening (TT #327, 2026-04-29):** Three parallel audits (UI/UX, piping, architecture) of the chain integration surfaced 5 P0s + ~12 P1s.  Closed via 7 wave PRs on PR #326.  Notable artifacts:
  ...
  - Plan + audit docs: [`docs/internal/AUDIT-k144-chain-2026-04-29.md`](docs/internal/AUDIT-k144-chain-2026-04-29.md), [`docs/internal/PLAN-k144-chain-hardening.md`](docs/internal/PLAN-k144-chain-hardening.md)
- **Wakeword + on-device dictation wave (TT #575 / PR #576, 2026-05-17):** follow-up to the Wave 1-7 closure that reuses the K144 audio + asr units in a new always-on configuration.  Branch `feat/wakeword` (commits `83f82e3` + `e5426cc`).  Adds `main/voice_wakeword.{c,h}` + `voice_m5_llm_wakeword_setup/_run/_teardown` + lifecycle hooks in `voice_onboard.c` (arms on warmup READY + every successful Wave 13 reset; tears down when vmode=4 chain takes the UART).  Live-verified wake on "tinker" on 2026-05-17; UI bridge (toast + orb ripple) added but needs hardware retest.  KWS unit dead-end (parse_config rejects every body) documented for vendor follow-up.  Full plan: [`docs/internal/PLAN-wakeword.md`](docs/internal/PLAN-wakeword.md).
...
For multi-week features (Grove, K144, widget platform), **write a plan doc first** (`docs/internal/PLAN-*.md`) with: hardware reality, code architecture, phased breakdown with file:line refs, honest unknowns, code anchors.  File a tracking issue that links to the doc as source-of-truth.  Branches die; docs + issues persist.
...
program, see [`docs/internal/AUDIT-state-of-stack-2026-05-11.md`](docs/internal/AUDIT-state-of-stack-2026-05-11.md).
...
**Plan:** [`docs/internal/PLAN-widget-platform.md`](./docs/internal/PLAN-widget-platform.md)
